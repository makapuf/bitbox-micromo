...
void Optionclick()
{
    ...
    switch (i) {
        case  0:
            if (--language < 0) language++;          // previous language
            SDL_WM_SetCaption(msg[language][0], NULL);
            break;
        case  1:
            if (++language == LANG_MAX) language--;  // next language
            SDL_WM_SetCaption(msg[language][0], NULL);
            break;
        case  2:
            x = xclient / xbitmap + 1;
        ...
        case  6:
            framedelay /= 2;
            if (framedelay < 1) framedelay = 1;    // more frames
            break;
        case  7:
            framedelay *= 2;
            if (framedelay > 64) framedelay = 64;  // less frames
            break;
        case  8:
